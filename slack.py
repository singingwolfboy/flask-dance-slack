...
@app.route("/")
def index():
    log.info("flask.session = %s", flask.session)
    log.info("slack_bp.token = %s", slack_bp.token)
    log.info("slack.token = %s", slack.token)
    log.info("slack.authorized = %s", slack.authorized)
